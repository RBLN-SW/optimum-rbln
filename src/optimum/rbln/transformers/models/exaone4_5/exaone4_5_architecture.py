...
from .configuration_exaone4_5 import RBLNExaone4_5_VisionPreTrainedModelConfig
...
class Exaone4_5VisionTransformerWrapper(nn.Module):
    def __init__(self, model: torch.nn.Module, rbln_config: RBLNExaone4_5_VisionPreTrainedModelConfig):
        super().__init__()
    ...
    def wrap_vision_blocks(
        ...
        window_seq_len: int,
        rbln_config: RBLNExaone4_5_VisionPreTrainedModelConfig,
    ):
        wrapped_blocks = []
...
class Exaone4_5VisionFullAttention(nn.Module):
    def __init__(self, model: nn.Module, rbln_config: RBLNExaone4_5_VisionPreTrainedModelConfig) -> None:
        super().__init__()
        self._origin_model = model
        self.rbln_config = rbln_config
        self.num_heads = model.num_heads
        self.num_key_value_heads = getattr(model, "num_key_value_groups", 1)
...
class Exaone4_5VisionWindowAttention(nn.Module):
    def __init__(
        self, model: nn.Module, window_seq_len: int, rbln_config: RBLNExaone4_5_VisionPreTrainedModelConfig
    ) -> None:
        super().__init__()
        self._origin_model = model
        self.rbln_config = rbln_config
...
class Exaone4_5Attention(DecoderOnlyAttention):
    def __post_init__(self, self_attn):
        ...
        # EXAONE-4.5 hybrid attention uses NoPE on full-attention layers
        # and RoPE only on sliding-attention layers.
        self.sliding_window = getattr(self_attn, "sliding_window", None)
        self.is_sliding = getattr(self_attn, "is_sliding", False)
