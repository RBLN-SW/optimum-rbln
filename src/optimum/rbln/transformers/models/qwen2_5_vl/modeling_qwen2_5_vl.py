...
class RBLNQwen2_5_VLModel(RBLNDecoderOnlyModel):
    ...
    def _preprocess_prefill(
        ...
        for b_idx in range(batch_size):
            ...
            second_per_grid_ts_slice = (
                second_per_grid_ts[video_idx : video_idx + video_nums] if second_per_grid_ts is not None else None
            )
            if is_transformers_version(">=", "5.0"):
                # v5 added a required `mm_token_type_ids` parameter
                # (text=0, image=1, video=2). Derive it from input_ids.
...
class RBLNQwen2_5_VLForConditionalGeneration(RBLNQwen2_5_VLModel, RBLNDecoderOnlyModelForCausalLM):
    ...
    def prepare_inputs_for_generation(
        ...
        mm_token_type_ids=None,  # v5 Qwen2_5_VLProcessor emits this; consume so generate doesn't flag it as unused.
        **kwargs,
    ):
        model_inputs = {}
