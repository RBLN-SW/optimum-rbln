...
class DecoderOnlyModel(nn.Module):
    ...
    def get_swa_custom_op_args(self, position_ids, query_position, mask_blocks: int = 1):
        ...
        valid_input_len = int(position_ids.shape[1])
        ...
        cache_update_end = torch.clamp(cache_offset.to(torch.int32), min=0, max=attn_mask_len)
        ...
        if mask_blocks > 1:
            ...
            cache_update_start = torch.where(
                ...
                torch.full_like(cache_update_end, right_window_start),
                torch.clamp(cache_update_end - max_cache_len, min=0),
            )
        else:
            cache_update_start = torch.clamp(cache_update_end - max_cache_len, min=0)

        positions = torch.arange(attn_mask_len, device=position_ids.device)[None, :]
        attn_mask = torch.where(
            (positions >= cache_update_start) & (positions < cache_update_end),
            1.0,
            0.0,
        )[:, None, None, :]
        attn_mask = attn_mask.expand(-1, 1, valid_input_len, -1)
