    ...
    def prefill_forward(
        ...
        for i, step in enumerate(range(0, query_length, self.rbln_config.prefill_chunk_size)):
            ...
            if self.rbln_config.use_attention_mask:
                if self.rbln_config.cache_impl == "sliding_window":
                    current_chunk_end = (
                        min(e, query_length) + prefix_cached_len
                        if self.rbln_config.use_position_ids
                        else e + prefix_cached_len
                    )
                    current_chunk_start, current_chunk_end = self._get_sliding_window_mask_range(current_chunk_end)

                    chunked_attention_mask.zero_()
                    if self.rbln_config.use_position_ids:
                        if current_chunk_end > current_chunk_start:
                            chunked_attention_mask[:, current_chunk_start:current_chunk_end] = 1
                    else:
                        chunked_attention_mask[:, :, :, current_chunk_start:current_chunk_end] = 1
                elif self.rbln_config.use_position_ids:
