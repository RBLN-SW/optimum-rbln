...
class RBLNDecoderOnlyFlashAttentionMixin:
    @classmethod
    def set_kvcache_num_blocks_before_compilation(
        cls,
        model: "PreTrainedModel",
        model_config: "PretrainedConfig",
        rbln_config: RBLNDecoderOnlyModelForCausalLMConfig,
    ):
        if is_compiler_supports_buffer_resize():
            # kvcache num blocks will be automatically determined after compilation.
            return

        logger.warning(
            "[KVCache] The installed version of rebel-compiler does not support automatic KV cache number of blocks "
            "determination based on available DRAM. The number of blocks is being estimated before "
            "compilation, which may be inaccurate in certain cases. "
            "To get more accurate number of blocks calculation, please upgrade rebel-compiler to the latest version. "
            "Alternatively, you can explicitly set 'kvcache_num_blocks' in rbln_config to suppress this warning."
        )
        ...
        # In case of memory is not enough for full blocks.
        if estimated_max_num_blocks < rbln_config.num_full_blocks:
            # In case of memory is not enough for full sequence length.
            if rbln_config.num_min_blocks > estimated_max_num_blocks:
                # NOTE: Just try to compile  with lower bound of blocks for full sequence length.
                # Even if it's larger than the estimated maximum number of blocks.
                logger.warning(
                    "[KVCache] Memory is not enough for full sequence length. "
                    "Here we will try to compile with lower bound of blocks for full sequence length. "
                    "However, this may cause a failure during model compilation "
                    "if the memory is still not enough."
                    "Please consider decreasing `max_seq_len` to reduce the number of blocks."
                )
                rbln_config.kvcache_num_blocks = rbln_config.num_min_blocks
            else:
